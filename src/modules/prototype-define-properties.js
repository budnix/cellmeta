...
/**
 * [CellMetaManagerPrototypeDefineProperties description]
 */
export function CellMetaManagerPrototypeDefineProperties() {
  const columnMetas = [];
  const cellMetas = [];
  ...
  this.createCellMeta = function(row, column) {
    ...
    if (!cellMetas[row][column]) {
      ...
      CellMetaManagerPrototypeDefineProperties.AVAILABLE_OPTIONS.forEach((option) => {
        Object.defineProperty(wrapper, option, {
          get() {
            return cellMeta[option];
          },
          set(value) {
            // changes.set(`${row}x${column}`, true);
            cellMeta[option] = value;
          },
        })
      });

      cellMetas[row][column] = wrapper;
    }
...
CellMetaManagerPrototypeDefineProperties.getInstance = function() {
  return new CellMetaManagerPrototypeDefineProperties();
};

CellMetaManagerPrototypeDefineProperties.AVAILABLE_OPTIONS = Object.keys(DefaultSettings.prototype);
